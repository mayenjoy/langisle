...
void QtSpeech::tell(QString text, QObject * obj, const char * slot) const
{
    if (d->waitingFinish)
	{
		...
		const_cast<QtSpeech *>(this)->killTimer(d->timerID);
		d->voice->Speak( NULL, SPF_PURGEBEFORESPEAK, NULL );
	}
    ...
    d->timerID = const_cast<QtSpeech *>(this)->startTimer(100);
...
void QtSpeech::setVoice(const QString &voiceName)
{
	ULONG count = 0;
	CComPtr<IEnumSpObjectTokens> voices;
	...
	for (int i =0; i< count; i++) {
		...
		SysCall( voices->Next( 1, &voice, NULL ), InitError);
		SysCall( SpGetDescription(voice, &w_name), InitError);
		QString vname = QString::fromWCharArray(w_name);
		if (vname == voiceName) 
		{
			d->voice->SetVoice(voice);
		}
		//DBG(qDebug() << "QtSpeech::setVoice" << vname);
		voice.Release();
	}
}
